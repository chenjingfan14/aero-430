...
for i = 1:N*M
    A(i,i)=(-40-8*deltaX(a)^2*k^2)/(12*deltaX(a)^2);
end 


for i=1:N-1
    for j=1:M
        A(i+(j-1)*N,i+(j-1)*N+1)=(8-deltaX(a)^2*k^2)/(12*deltaX(a)^2);
        A(i+(j-1)*N+1,i+(j-1)*N)=(8-deltaX(a)^2*k^2)/(12*deltaX(a)^2);
    end
end

for i=1:N
    for j=1:M-1
        A(i+(j-1)*N, i+j*N)=(8-deltaX(a)^2*k^2)/(12*deltaX(a)^2);
        A(i+j*N, i+(j-1)*N)=(8-deltaX(a)^2*k^2)/(12*deltaX(a)^2);
    end
end

for i=1:N-1
    for j=1:M-1
         A(i+(j-1)*N, i+j*N+1)=2/(12*deltaX(a)^2);
         A(i+(j-1)*N+1, i+j*N)=2/(12*deltaX(a)^2);
         A(i+j*N+1, i+(j-1)*N)=2/(12*deltaX(a)^2);
         A(i+j*N, i+(j-1)*N+1)=2/(12*deltaX(a)^2);
    end
end
...
zlabel('u')
title(sprintf('Fourth Order Finite Difference for k=20,deltaX=deltaY=%d',deltaX(a)));
 colorbar('location','eastoutside');
...
title('Fourth Order Error vs StepSize Log Plot')
...
urichard = zeros(1,d);
for i=2:d-1
    urichard(i)=(umidapproximatesecondorder(i)^2-umidapproximatesecondorder(i-1)*umidapproximatesecondorder(i+1))/(2*umidapproximatesecondorder(i)-umidapproximatesecondorder(i-1)-umidapproximatesecondorder(i+1));
end

richardsonerror=zeros(1,d);
for i=2:d-1
    richardsonerror(i)=abs(umidexact-urichard(i));
end


betarichardson = zeros(1,d);
for i=2:d-1
betarichardson(i)=(1/log(2))*log((urichard(i)-umidapproximatesecondorder(i-1))/(urichard(i)-umidapproximatesecondorder(i)));
end 

Table1(:,4)=[urichard'];     
Table1(:,5)=[richardsonerror'];
Table1(:,6)=[betarichardson'];
